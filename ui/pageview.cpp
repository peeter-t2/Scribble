...
QString PageViewPrivate::selectedProps() const
/*{
    if ( pagesWithTextSelection.isEmpty() )
        return QString();
...
}*/
{
    ...
    else
    {
        ...
        {
            //pg = document->page( selpages.at( i ) );
            text.append( selpages.at( i ) );
        }
        //pg = document->page( selpages.last() );
        text.append( selpages.last() );
    }
    // t88tab kindlasti siitkaudu text = "test";
    text = typeid(selpages.first()).name();
    return text; // nyyd ta v4hemalt ei t88ta, aga kuidas numbrid ymber teha
}
...
void PageView::copyPropsSelection() const
{
    const QString text = d->selectedProps();
    if ( !text.isEmpty() )
    {
        QClipboard *cb = QApplication::clipboard();
        cb->setText( text, QClipboard::Clipboard );
...
//                    textSelectionClear();
                ...
                else if ( !d->mousePressPos.isNull() && rightButton )
                {
                    ...
                    {
                        KMenu menu( this );
                        QAction *tagTheText = menu.addAction( KIcon( "text-speak" ), i18n( "Tag the Text!" ) );
                        ...
                        if ( choice )
                        {
                        	if ( choice == tagTheText )
                        	    copyPropsSelection();
                        	else if ( choice == textToClipboard )
                                copyTextSelection();
                            else if ( choice == speakText )
                            {
                                const QString text = d->selectedText();
...
void PageView::updateSelection( const QPoint & pos )
{
    ...
    else if ( d->mouseTextSelecting)
    {
        scrollPosIntoView( pos );
        int first = -1;
        const QList< Okular::RegularAreaRect * > selections = textSelections( pos, d->mouseSelectPos, first ); // juba siit???
